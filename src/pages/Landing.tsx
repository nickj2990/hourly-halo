...
export default function Landing() {
  return (
    <div style={{ fontFamily: "'Syne', 'Inter', sans-serif", background: '#080c14', color: '#e8eaf0', minHeight: '100vh', overflowX: 'hidden' }}>
      ...
      {/* Glow blobs */}
      <div style={{ position: 'fixed', top: '-20vh', left: '-10vw', width: '60vw', height: '60vw', borderRadius: '50%', background: 'radial-gradient(circle, rgba(16,185,129,.10) 0%, transparent 70%)', zIndex: 0, pointerEvents: 'none' }} />
      <div style={{ position: 'fixed', bottom: '-10vh', right: '-10vw', width: '40vw', height: '40vw', borderRadius: '50%', background: 'radial-gradient(circle, rgba(16,185,129,.06) 0%, transparent 70%)', zIndex: 0, pointerEvents: 'none' }} />
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 40px', height: 64, backdropFilter: 'blur(16px)', position: 'sticky', top: 0, zIndex: 50, background: 'rgba(8,12,20,.75)' }}>
          ...
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 1, background: 'linear-gradient(90deg, transparent 0%, rgba(16,185,129,.4) 30%, rgba(52,211,153,.4) 70%, transparent 100%)' }} />
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 1, userSelect: 'none' }}>
            ...
            <span style={{ fontWeight: 900, fontSize: 20, letterSpacing: '-0.04em', color: '#10b981', lineHeight: 1 }}>/</span>
          ...
          <Link to="/auth"
            style={{ position: 'relative', overflow: 'hidden', color: '#e2e8f0', padding: '8px 22px', borderRadius: 9, fontWeight: 600, fontSize: 14, textDecoration: 'none', letterSpacing: '-0.01em', border: '1px solid rgba(16,185,129,.4)', background: 'transparent', transition: 'color .2s, border-color .2s', display: 'inline-flex', alignItems: 'center', gap: 7 }}
            onMouseEnter={e => {
              (e.currentTarget as HTMLElement).style.borderColor = 'rgba(16,185,129,.8)';
        ...
        <section style={{ minHeight: '92vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '80px 40px 60px', maxWidth: 1100, margin: '0 auto', position: 'relative' }}>
          ...
          {/* Live timer badge */}
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(16,185,129,.10)', border: '1px solid rgba(16,185,129,.25)', borderRadius: 100, padding: '6px 14px', marginBottom: 36, width: 'fit-content' }}>
            <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#10b981', display: 'inline-block', animation: 'pulse 2s infinite' }} />
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 13, color: '#34d399', letterSpacing: '0.05em' }}>
              <TickingClock />
            </span>
          </div>
          ...
          <h1 style={{ fontSize: 'clamp(44px, 7vw, 88px)', fontWeight: 800, lineHeight: 1.0, letterSpacing: '-0.04em', marginBottom: 28, maxWidth: 800, color: '#fff' }}>
            Your time<br />
            <span style={{ color: '#34d399', position: 'relative' }}>
              is money.
              <svg style={{ position: 'absolute', bottom: -4, left: 0, width: '100%', height: 8, overflow: 'visible' }} viewBox="0 0 200 8" preserveAspectRatio="none">
                <path d="M0,6 Q50,0 100,6 Q150,12 200,6" fill="none" stroke="#10b981" strokeWidth="2.5" strokeLinecap="round" />
              </svg>
            </span>
            <br />Track both.
          </h1>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            <Link to="/auth" style={{ background: '#10b981', color: '#fff', padding: '14px 32px', borderRadius: 10, fontWeight: 700, fontSize: 16, textDecoration: 'none', letterSpacing: '-0.01em', transition: 'transform .15s, box-shadow .15s', boxShadow: '0 0 32px rgba(16,185,129,.35)' }}
              onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 40px rgba(16,185,129,.5)'; }}
              onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 0 32px rgba(16,185,129,.35)'; }}>
              Start tracking free
            </Link>
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 64, flexWrap: 'wrap' }}>
            {[['∞', 'Clients'], ['∞', 'Projects'], ['PDF', 'Export'], ['0%', 'Commission']].map(([val, label]) => (
              <div key={label} style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 10, padding: '12px 20px', display: 'flex', flexDirection: 'column', gap: 2 }}>
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: val === '∞' ? 28 : 20, fontWeight: 600, color: '#34d399', lineHeight: 1 }}>{val}</span>
        ...
        {/* DIVIDER */}
        <div style={{ borderTop: '1px solid rgba(16,185,129,.12)', maxWidth: 1100, margin: '0 auto' }} />

        {/* FEATURES */}
        <section style={{ padding: '100px 40px', maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 16, marginBottom: 60 }}>
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: '#10b981', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Features</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 2 }}>
            {FEATURES.map((f) => (
              <div key={f.number} style={{ background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.07)', borderRadius: 16, padding: '32px 28px', transition: 'background .2s, border-color .2s', cursor: 'default' }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(16,185,129,.07)'; (e.currentTarget as HTMLElement).style.borderColor = 'rgba(16,185,129,.3)'; }}
        ...
        {/* HOW IT WORKS */}
        <section id="how-it-works" style={{ padding: '100px 40px', background: 'rgba(16,185,129,.04)', borderTop: '1px solid rgba(16,185,129,.1)', borderBottom: '1px solid rgba(16,185,129,.1)' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 16, marginBottom: 60 }}>
              <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: '#10b981', letterSpacing: '0.1em', textTransform: 'uppercase' }}>How it works</span>
              <div style={{ flex: 1, height: 1, background: 'rgba(16,185,129,.2)' }} />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 40 }}>
              {STEPS.map((s, i) => (
                <div key={s.n} style={{ position: 'relative' }}>
                  {i < STEPS.length - 1 && (
                    <div style={{ position: 'absolute', top: 24, left: 'calc(100% - 20px)', width: 40, height: 1, background: 'rgba(16,185,129,.3)', display: 'none' }} className="step-connector" />
                  )}
                  <div style={{ width: 48, height: 48, borderRadius: 12, border: '1px solid rgba(16,185,129,.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 24, background: 'rgba(16,185,129,.08)' }}>
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 18, fontWeight: 700, color: '#10b981' }}>{s.n}</span>
                  </div>
        ...
        <section style={{ padding: '100px 40px', maxWidth: 680, margin: '0 auto', textAlign: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 16, marginBottom: 60 }}>
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: '#10b981', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Pricing</span>
            <div style={{ flex: 1, height: 1, background: 'rgba(16,185,129,.2)' }} />
          </div>
          <div style={{ background: 'rgba(16,185,129,.06)', border: '1px solid rgba(16,185,129,.25)', borderRadius: 20, padding: '52px 40px', position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: -40, right: -40, width: 200, height: 200, borderRadius: '50%', background: 'radial-gradient(circle, rgba(16,185,129,.12) 0%, transparent 70%)', pointerEvents: 'none' }} />
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 60, fontWeight: 700, color: '#10b981', letterSpacing: '-0.04em', marginBottom: 8 }}>$0</div>
              ...
              {['Unlimited time entries', 'Unlimited clients & projects', 'PDF invoice export', 'Email invoice delivery', 'Revenue reports & CSV export', 'Timer rounding rules'].map(item => (
                <li key={item} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 14, color: '#cbd5e1', fontFamily: "'Inter', sans-serif" }}>
                  <svg viewBox="0 0 16 16" fill="none" width="16" height="16">
                    <circle cx="8" cy="8" r="7" fill="rgba(16,185,129,.2)" />
            ...
            <Link to="/auth" style={{ display: 'inline-block', background: '#10b981', color: '#fff', padding: '14px 40px', borderRadius: 10, fontWeight: 700, fontSize: 16, textDecoration: 'none', letterSpacing: '-0.01em', boxShadow: '0 0 32px rgba(16,185,129,.35)', transition: 'transform .15s, box-shadow .15s' }}
              onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 40px rgba(16,185,129,.5)'; }}
              onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 0 32px rgba(16,185,129,.35)'; }}>
              Get started free
            </Link>
        ...
        <footer style={{ borderTop: '1px solid rgba(255,255,255,.06)', padding: '36px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 1, userSelect: 'none' }}>
            ...
            <span style={{ fontWeight: 900, fontSize: 16, letterSpacing: '-0.04em', color: '#10b981', lineHeight: 1 }}>/</span>
